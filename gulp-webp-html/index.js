...
module.exports = function(extensions){
    ...
    return through.obj(function(file, enc, cb) {
        ...
        try {
            var pictureRender = function (url, imgTag, isDataSrc) {
					if(isDataSrc){
						return '<picture><source class="lazyload" data-srcset="' + url + '" type="image/webp">' + imgTag + '</picture>';
					}else{
            ...
            const data = file.contents.toString()
                    ...
                    .map(function(line){
                        ...
                        if((line.indexOf('<img') + 1) && !inPicture){
									...
									var ReDataSrc = /<img([^>]*)data-src=\"(\S+)\"([^>]*)>/ig;
									var ReSrc = /<img([^>]*)src=\"(\S+)\"([^>]*)>/ig;
									var regexpArray = ReDataSrc.exec(line);
									var isDataSrc = true;

									if(!regexpArray){
										regexpArray = ReSrc.exec(line);
										isDataSrc = false;
									}
									...
									// Компилим <picture/>
									var newHTML = pictureRender(newUrl,imgTag, isDataSrc);
